 ...
 * superclass AbstractMarsToolAndApplication. Pete Sanderson, verison 1.0, 23
 * December 2010.
 */
public class BitmapDisplay extends AbstractMarsToolAndApplication {
  ...
  private JComboBox visualizationUnitPixelWidthSelector,
      visualizationUnitPixelHeightSelector, visualizationPixelWidthSelector,
      visualizationPixelHeightSelector, displayBaseAddressSelector;
  ...
  private final String[] visualizationUnitPixelWidthChoices = {"1", "2",  "4",
                                                               "8", "16", "32"};
  ...
  private final String[] displayAreaPixelWidthChoices = {"64", "128", "256",
                                                         "512", "1024"};
  private final int defaultDisplayWidthIndex = 3;
  private final String[] displayAreaPixelHeightChoices = {"64", "128", "256",
                                                          "512", "1024"};
  ...
  private int unitPixelWidth =
      Integer.parseInt(visualizationUnitPixelWidthChoices
  ...
  private int displayAreaWidthInPixels =
      Integer.parseInt(displayAreaPixelWidthChoices[defaultDisplayWidthIndex]);
  private int displayAreaHeightInPixels = Integer.parseInt(
      displayAreaPixelHeightChoices[defaultDisplayHeightIndex]);
   ...
   *     window.
   */
  public BitmapDisplay(String title, String heading) { super(title, heading); }

  /**
   *  Simple constructor, likely used by the MARS Tools menu mechanism
   */
  public BitmapDisplay() { super("Bitmap Display, " + version, heading); }

  /**
   * Main provided for pure stand-alone use.  Recommended stand-alone use is to
   * write a driver program that instantiates a Bitmap object then invokes its
   * go() method. "stand-alone" means it is not invoked from the MARS Tools
   * menu.  "Pure" means there is no driver program to invoke the application.
   */
   ...
   */
  public String getName() { return "Bitmap Display"; }

  /**
   *  Override the inherited method, which registers us as an Observer over the
   * static data segment (starting address 0x10010000) only.  This version will
   * register us as observer over the the memory range as selected by the base
   * address combo box and capacity of the visualization display (number of
   * visualization elements times the number of memory words each one
   * represents). It does so by calling the inherited 2-parameter overload of
   * this method. If you use the inherited GUI buttons, this method is invoked
   * when you click "Connect" button on MarsTool or the "Assemble and Run"
   * button on a Mars-based app.
   */
  protected void addAsObserver() {
    int highAddress = baseAddress + theGrid.getRows() * theGrid.getColumns() *
    ...
    addAsObserver(baseAddress - 4, highAddress);
  }
  ...
  //////////////////////////////////////////////////////////////////////////////////////
  //  Rest of the protected methods.  These override do-nothing methods
  ...
  protected void processMIPSUpdate(Observable memory,
    ...
    if (accessNotice.getAccessType() == AccessNotice.WRITE) {
      MemoryAccessNotice MemAccess = (MemoryAccessNotice)accessNotice;

      if (MemAccess.getAddress() ==
          this.baseAddress - Memory.WORD_LENGTH_BYTES) {
        this.canvas.repaint();
        this.theWindow.repaint();
      } else {
        updateColorForAddress(MemAccess);
      }
    }
  }
  ...
  protected void initializePreGUI() {
    ...
    //       several combo boxes that have not been created yet.  But a
    //       default grid needs to be allocated for initial canvas display.
    theGrid = new Grid(displayAreaHeightInPixels / unitPixelHeight,
                       displayAreaWidthInPixels / unitPixelWidth);
  }

  /**
   *  The only post-GUI initialization is to create the initial Grid object
   * based on the default settings of the various combo boxes. Overrides
   ...
   * processed, after display configuration changes as needed, and after each
   * execution step when Mars is running in timed mode.  Overrides inherited
  ...
  // UI components and layout for left half of GUI, where settings are
  // specified.
  private JComponent buildOrganizationArea() {
    ...
    visualizationUnitPixelWidthSelector =
        new JComboBox(visualizationUnitPixelWidthChoices);
    visualizationUnitPixelWidthSelector.setEditable(false);
    visualizationUnitPixelWidthSelector.setBackground(backgroundColor);
    visualizationUnitPixelWidthSelector.setSelectedIndex(
        defaultVisualizationUnitPixelWidthIndex);
    visualizationUnitPixelWidthSelector.setToolTipText(
    ...
    visualizationUnitPixelWidthSelector.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        unitPixelWidth =
            getIntComboBoxSelection(visualizationUnitPixelWidthSelector);
        theGrid = createNewGrid();
        canvas.repaint();
      }
    });
    visualizationUnitPixelHeightSelector =
        new JComboBox(visualizationUnitPixelHeightChoices);
    ...
    visualizationUnitPixelHeightSelector.setSelectedIndex(
        defaultVisualizationUnitPixelHeightIndex);
    visualizationUnitPixelHeightSelector.setToolTipText(
        "Height in pixels of rectangle representing memory word");
    visualizationUnitPixelHeightSelector.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            unitPixelHeight =
                getIntComboBoxSelection(visualizationUnitPixelHeightSelector);
            theGrid = createNewGrid();
            canvas.repaint();
          }
        });
    visualizationPixelWidthSelector =
        new JComboBox(displayAreaPixelWidthChoices);
    ...
    visualizationPixelWidthSelector.setToolTipText(
    ...
    visualizationPixelWidthSelector.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        displayAreaWidthInPixels =
            getIntComboBoxSelection(visualizationPixelWidthSelector);
    ...
    visualizationPixelHeightSelector.setSelectedIndex(
        defaultDisplayHeightIndex);
    visualizationPixelHeightSelector.setToolTipText(
    ...
    visualizationPixelHeightSelector.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        displayAreaHeightInPixels =
            getIntComboBoxSelection(visualizationPixelHeightSelector);
        canvas.setPreferredSize(getDisplayAreaDimension());
        canvas.setSize(getDisplayAreaDimension());
        theGrid = createNewGrid();
        canvas.repaint();
    ...
    displayBaseAddressSelector.setToolTipText(
        "Base address for display area (upper left corner)");
    displayBaseAddressSelector.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        // This may also affect what address range we should be registered as
        // an Observer for.  The default (inherited) address range is the MIPS
        // static data segment starting at 0x10010000. To change this requires
        // override of AbstractMarsToolAndApplication.addAsObserver().  The
        // no-argument version of that method is called automatically  when
        // "Connect" button is clicked for MarsTool and when "Assemble and
        // Run" button is clicked for Mars application.
        updateBaseAddress();
        // If display base address is changed while connected to MIPS (this
        // can only occur when being used as a MarsTool), we have to delete
        // ourselves as an observer and re-register.
    ...
    unitWidthInPixelsRow.add(new JLabel("Unit Width in Pixels "),
                             BorderLayout.WEST);
    unitWidthInPixelsRow.add(visualizationUnitPixelWidthSelector,
                             BorderLayout.EAST);

    JPanel unitHeightInPixelsRow = getPanelWithBorderLayout();
    unitHeightInPixelsRow.setBorder(emptyBorder);
    unitHeightInPixelsRow.add(new JLabel("Unit Height in Pixels "),
                              BorderLayout.WEST);
    unitHeightInPixelsRow.add(visualizationUnitPixelHeightSelector,
                              BorderLayout.EAST);

    JPanel widthInPixelsRow = getPanelWithBorderLayout();
    widthInPixelsRow.setBorder(emptyBorder);
    widthInPixelsRow.add(new JLabel("Display Width in Pixels "),
                         BorderLayout.WEST);
    ...
    heightInPixelsRow.add(new JLabel("Display Height in Pixels "),
                          BorderLayout.WEST);
    ...
    baseAddressRow.add(new JLabel("Base address for display "),
                       BorderLayout.WEST);
  ...
  // For greatest flexibility, initialize the display base choices directly
  // from the constants defined in the Memory class.  This method called prior
  // to building the GUI.  Here are current values from Memory.java:
  ...
  private void updateBaseAddress() {
    baseAddress =
        displayBaseAddresses[displayBaseAddressSelector.getSelectedIndex()];
    /*  If you want to extend this app to allow user to edit combo box, you
       can always parse the getSelectedItem() value, because the pre-defined
       items are all formatted such that the first 10 characters contain the
       integer's hex value.  And if the value is user-entered, the numeric
       part cannot exceed 10 characters for a 32-bit address anyway.  So if
       the value is > 10 characters long, slice off the first 10 and apply
       Integer.parseInt() to it to get custom base address.
    */
  }
  ...
  private void resetCounts() { theGrid.reset(); }
  ...
  private int getIntComboBoxSelection(JComboBox comboBox) {
    try {
      ...
      // Can occur only if initialization list contains badly formatted
      // numbers. This is a developer's error, not a user error, and better be
  ...
  private void updateColorForAddress(MemoryAccessNotice notice) {
    ...
    try {
      theGrid.setElement(offset / theGrid.getColumns(),
                         offset % theGrid.getColumns(), value);
  ...
  private class GraphicsPanel extends JPanel {
    ...
    // override default paint method to assure display updated correctly every
    // time
    // the panel is repainted.
    public void paint(Graphics g) { paintGrid(g, theGrid); }
  ...
  private class Grid {
    ...
    // Returns value in given grid element; null if row or column is out of
    // range.
    ...
    // checking. Is faster than getElement but will throw array index out of
    // bounds exception if parameter values are outside the bounds of the
    // grid.
